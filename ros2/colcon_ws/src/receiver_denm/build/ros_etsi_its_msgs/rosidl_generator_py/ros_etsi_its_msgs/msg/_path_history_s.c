...
bool ros_etsi_its_msgs__msg__path_history__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("ros_etsi_its_msgs.msg._path_history.PathHistory", full_classname_dest, 47) == 0);
  }
  ros_etsi_its_msgs__msg__PathHistory * ros_message = _ros_message;
  {  // points
    ...
    if (!ros_etsi_its_msgs__msg__PathPoint__Sequence__init(&(ros_message->points), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create ros_etsi_its_msgs__msg__PathPoint__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    ros_etsi_its_msgs__msg__PathPoint * dest = ros_message->points.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!ros_etsi_its_msgs__msg__path_point__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * ros_etsi_its_msgs__msg__path_history__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("ros_etsi_its_msgs.msg._path_history");
  ...
  {  // points
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = ros_etsi_its_msgs__msg__path_point__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
