...
class GPSPublisher(Node):
    ...
    def listen_for_gps_data(self):
        while rclpy.ok():
            ...
            nmea_sentence = data.decode('utf-8').strip()  # Decode the byte data
            self.get_logger().info(f"Received NMEA Sentence: {nmea_sentence}")
            ...
            if nmea_sentence.startswith("$GPRMC"):
                ...
                if len(parts) > 2 and parts[2] == 'A':  # Check if the data is valid
                    try:
                        ...
                        lat_micro = int(lat_decimal * 1_000_000)
                        lon_micro = int(lon_decimal * 1_000_000)
                    ...
                    try:
                        if parts[2]:  # Check if latitude is provided
                            ...
                            if parts[4]:  # Check if longitude is provided
                                ...
                                self.get_logger().info(f"lon_decimallon_decimallon_decimallon_decimallon_decimal: Latitude (microdegrees): {lat_decimal}, Longitude (microdegrees): {lon_decimal}")
